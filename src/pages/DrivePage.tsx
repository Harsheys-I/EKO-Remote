...
export function DrivePage({ client, state, settings, stats, mock, onSettings }: { client: EkoClient | null; state: RobotState; settings: RuntimeSettings | null; stats: LinkStats; mock: MockHardwareController; onSettings: (settings: RuntimeSettings) => void }) {
  ...
  useEffect(() => {
    if (!gamepadEnabled) { setGamepadName(null); setGamepadVector({ vx: 0, vy: 0, wz: 0 }); return; }
    let frame = 0;
    let last = "";
    ...
    const poll = () => {
      ...
      if (gamepad) {
        ...
        let rotation = 0;
        if (gamepad.buttons[5]?.pressed) rotation = 0.5;
        if (gamepad.buttons[4]?.pressed) rotation = -0.5;
        const next = { vx: deadzone(gamepad.axes[0] ?? 0), vy: deadzone(-(gamepad.axes[1] ?? 0)), wz: rotation };
        const signature = `${next.vx.toFixed(3)}:${next.vy.toFixed(3)}:${next.wz.toFixed(3)}`;
        if (signature !== last) { last = signature; setGamepadVector(next); }
      } else {
        setGamepadName(null);
        if (last !== "0:0:0") { last = "0:0:0"; setGamepadVector({ vx: 0, vy: 0, wz: 0 }); }
      }
  ...
  return <div className="page-grid drive-page">{!hardwareReady && <div className="safety-banner"><ShieldAlert size={20} /><div><strong>Movement controls are locked</strong><span>{!stats.connected ? "Connect to EKO before driving." : settings?.control_mode === "stationary" ? "Stationary mode is active. Select manual or assist to move." : !settings?.hardware_enabled && !mock.active ? "Enable Mock Mode on the Dashboard, or acknowledge physical hardware in Config and restart EKO." : "The browser mock hardware channel is not ready."}</span></div></div>}
