...
`health`, `events`, `memories`, `memory.remember`, `memory.forget`, `message`, `command`, `drive`, `control`, `settings.get`, `settings.update`, `ai`, and `vision.snapshot` map directly to EKO's v0.2.0 control API.
