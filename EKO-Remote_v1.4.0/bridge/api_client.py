...
class EkoAPI:
    ...
    def _route(operation: str, payload: dict[str, Any]) -> tuple[str, str, dict[str, Any] | None]:
        ...
        if operation == "navigation.status":
            return "GET", "/navigation/status", None
        if operation == "navigation.plan":
            return "POST", "/navigation/plan", {"destination": payload.get("destination")}
        if operation == "navigation.go":
            return "POST", "/navigation/go", {
                "destination": payload.get("destination"),
                "return_to_start": bool(payload.get("return_to_start", False)),
            }
        if operation == "navigation.stop":
            return "POST", "/navigation/stop", {}
        if operation == "navigation.relocalize":
            return "POST", "/navigation/relocalize", {}
        if operation == "navigation.obstacles.clear":
            return "POST", "/navigation/obstacles/clear", {}
