...
export interface StatusPayload {
  ...
  capability_status?: {
    ...
    relocalization?: RelocalizationStatus;
  };
...
export interface MapPayload {
  ...
  daily_path?: NavigationDay | null;
}

export interface NavigationWaypoint { x_m: number; y_m: number; }
export interface NavigationSegment { from: NavigationWaypoint; to: NavigationWaypoint; distance_m: number; heading_deg: number; turn_to_next_deg: number; corner_speed_m_s: number; }
export interface NavigationRoute {
  start: NavigationWaypoint;
  goal: NavigationWaypoint;
  destination?: { x_m: number; y_m: number; label: string; kind: string; id?: string };
...
export interface NavigationObstacle { id: string; x_m: number; y_m: number; radius_m: number; confidence: number; observations: number; last_seen: number; source: string; }
export interface NavigationTrip { id: string; status: string; started_at: number; ended_at: number | null; travelled_distance_m: number; path: Array<MapPose & { timestamp?: number }>; goal: Record<string, unknown>; }
export interface NavigationDay { date: string; distance_m: number; trips: NavigationTrip[]; }
export interface NavigationStatus {
  ...
  memory: { active_trip: NavigationTrip | null; obstacles: NavigationObstacle[]; folder: string };
}
export interface RelocalizationStatus { enabled: boolean; running: boolean; state: string; last_result: Record<string, unknown> | null; last_error: string | null; }
export interface NavigationPlanPayload { ok: boolean; plan: NavigationRoute; }
...
export interface CameraEvidence { mime_type: string; image_base64: string; sha256: string; bytes: number; frame_count: number; }
...
export type EkoOperation =
  ...
  | "navigation.status" | "navigation.plan" | "navigation.go" | "navigation.stop"
  | "navigation.relocalize" | "navigation.obstacles.clear"
