...
export class EkoRequestError extends Error {
  constructor(message: string, readonly payload: Record<string, unknown>) {
    super(message);
    this.name = "EkoRequestError";
  }
}
...
export class WifiTransport extends TransportBase implements EkoTransport {
  ...
  async request<T>(operation: EkoOperation, payload: Record<string, unknown> = {}): Promise<T> {
    ...
    if (!response.ok) throw new EkoRequestError(
      value.error ?? value.response ?? `EKO returned HTTP ${response.status}`,
  ...
  private route(operation: EkoOperation, payload: Record<string, unknown>): Route {
    switch (operation) {
      ...
      case "navigation.status": return { path: "/navigation/status" };
      case "navigation.plan": return { path: "/navigation/plan", method: "POST", body: { destination: payload.destination } };
      case "navigation.go": return { path: "/navigation/go", method: "POST", body: { destination: payload.destination, return_to_start: payload.return_to_start } };
      case "navigation.stop": return { path: "/navigation/stop", method: "POST", body: {} };
      case "navigation.relocalize": return { path: "/navigation/relocalize", method: "POST", body: {} };
      case "navigation.obstacles.clear": return { path: "/navigation/obstacles/clear", method: "POST", body: {} };
