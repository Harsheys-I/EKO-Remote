...
describe("EkoClient temporary CHAT operations", () => {
  ...
  it("uses dedicated navigation, route, stop, and relocalization operations", async () => {
    const link = transport();
    const client = new EkoClient(link.value);
    await client.planNavigation("Kitchen");
    await client.navigate({ x_m: 2, y_m: 3, label: "Hall" }, true);
    await client.stopNavigation();
    await client.relocalize();
    await client.clearNavigationObstacles();
    expect(link.request).toHaveBeenCalledWith("navigation.plan", { destination: "Kitchen" });
    expect(link.request).toHaveBeenCalledWith("navigation.go", {
      destination: { x_m: 2, y_m: 3, label: "Hall" },
      return_to_start: true,
    });
    expect(link.request).toHaveBeenCalledWith("navigation.stop");
    expect(link.request).toHaveBeenCalledWith("navigation.relocalize");
    expect(link.request).toHaveBeenCalledWith("navigation.obstacles.clear");
  });
});
