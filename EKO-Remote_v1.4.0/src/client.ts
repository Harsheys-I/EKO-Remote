import type { ConfigBatchPayload, ConfigListPayload, ConfigSavePayload, ControlStatus, DebugLogsPayload, DriveVector, EkoEvent, EyesStatus, FacesPayload, FloorPlan, FollowStatus, MapPayload, MemoryRecord, NavigationPlanPayload, NavigationStatus, RuntimeSettings, ServiceResult, StatusPayload, TemporaryChatStatus, TerminalStatus, VisionSnapshot, WifiProfilesPayload, WifiRecoveryProfileDraft } from "./types";
...
export class EkoClient {
  ...
  navigationStatus() { return this.transport.request<NavigationStatus>("navigation.status"); }
  planNavigation(destination: string | { x_m: number; y_m: number; label?: string }) { return this.transport.request<NavigationPlanPayload>("navigation.plan", { destination }); }
  navigate(destination: string | { x_m: number; y_m: number; label?: string }, returnToStart = false) { return this.transport.request<ServiceResult>("navigation.go", { destination, return_to_start: returnToStart }); }
  stopNavigation() { return this.transport.request<ServiceResult>("navigation.stop"); }
  relocalize() { return this.transport.request<ServiceResult>("navigation.relocalize"); }
  clearNavigationObstacles() { return this.transport.request<{ ok: boolean }>("navigation.obstacles.clear"); }
