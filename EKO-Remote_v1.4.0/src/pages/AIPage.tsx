...
import type { CameraEvidence, CapabilityQuotaStatus, EkoEvent, LinkStats, RuntimeSettings, StatusPayload, TemporaryChatStatus } from "../types";

type ChatItem = { role: "user" | "eko"; text: string; at: Date; image?: CameraEvidence };

function cameraEvidence(metadata: Record<string, unknown>): CameraEvidence | undefined {
  const value = metadata.camera_evidence;
  if (!value || typeof value !== "object") return undefined;
  const evidence = value as Partial<CameraEvidence>;
  return typeof evidence.image_base64 === "string" && typeof evidence.mime_type === "string"
    ? evidence as CameraEvidence
    : undefined;
}
...
export function AIPage({ client, status, events, stats, onSettings }: {
  ...
  const send = async (event: FormEvent) => {
    ...
    try {
      ...
      setChat((items) => [...items, { role: "eko", text: result.response ?? "Request completed.", at: new Date(), image: cameraEvidence(result.metadata) }]);
      setHistory(await client.chatHistory());
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : "Request failed";
      const payload = cause && typeof cause === "object" && "payload" in cause
        ? (cause as { payload?: Record<string, unknown> }).payload
        : undefined;
